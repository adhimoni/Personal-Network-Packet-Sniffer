...
def co_ordinates():
    return math.floor((random.randint(1,1)/random.randint(1,2))+1)

source_mac_address_json = []
label=[]
x_coordinate=[]
y_coordinate=[]
result= {"nodes": [{"id": source_mac_address_json ,"label": label,"x": x_coordinate,"y": y_coordinate,"size": 3}]}
...
while True:
	...
	while game==True:
		...
		ethernet_protocol = socket.ntohs(eth[2])
		packet_capture_time = datetime.datetime.now()
		source_mac_address = ethernet_address(packet_data[6:12])
		source_mac_address_json.append(source_mac_address)
		x_coordinate.append(co_ordinates())
		y_coordinate.append(co_ordinates())
		
		print("\n---------------------------------Parsed Data---------------------------------")
		print'Packet Capture Time:\t' + str(packet_capture_time)
		print'Destination MAC:\t' + ethernet_address(packet_data[0:6])
		print'Source MAC:\t\t' + str(source_mac_address)
